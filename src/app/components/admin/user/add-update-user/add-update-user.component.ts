...
getUserById(id: any) {
  this._adminService.getUserById(id).subscribe({
    next: (result: any) => {
      ...
      //   userData.agent.forEach((agent: any) => {
   ...
   getAllRoleListWma() {
    this._adminService.getAllRoleListWma().subscribe({
      next: (res: any) => {
       ...
       this.agentRoleList = this.allRoleList.filter((d: any) => {
          const role = d.role_name?.trim().toLowerCase();
          return role !== 'customer' && role !== 'super admin' && role !== 'supper admin';
        });

        console.log('Filtered Role List:', this.agentRoleList);
