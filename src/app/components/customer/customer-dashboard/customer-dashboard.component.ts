...
import { Chart, registerables } from 'chart.js'; // ✅ ADD THIS IMPORT
...
tabLabels = ['open', 'in progress', 'on hold', 'resolved', 'closed'];
 ...
 this.getStatusWiseTicketList(this.selectedStatus);
  ...
  getAdminTaskStatusCount(id:any) {
    this._customerService.getDashBoardTicketStatusCount(id,this.customer_id).subscribe({
      next: (res: any) => {
  ...
  getTaskStatusCount(id:any) {
    this._customerService.getTicketStatusCount(id,this.customer_id).subscribe({
      next: (res: any) => {
        if (res.data) {
